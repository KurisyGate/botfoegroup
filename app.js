console.log('[botcotvkedax|Котэ бот] >> Loading bot. Please, wait! // by vk.com/temadobriy');
...
let user = new VK();
user.setOptions({
	token: 'a991abba8740228236f7531e073476b8e99d383352c540a91e80740caf9f476a0a6fb72750dcb7d6107f5'
});
...
let chatid = message.chatId;	
		...
		message.send(`😜 || Приветствую тебя, игрок - ${user[0].first_name}! Это крутой игровой бот Котэ .\n\n😃 >> Чтобы начать играть, нажмите нужную кнопку, ну или написать команду - Помощь`, 
		{
...
vk.setOptions({ token: 'a991abba8740228236f7531e073476b8e99d383352c540a91e80740caf9f476a0a6fb72750dcb7d6107f5', pollingGroupId: 184991221 });
...
cmd.hear(/^(?:eval)\s([^]+)$/i, async (message, bot) => {
	message.user.foolder += 1;
		if(message.user.right < 4) return bot(`доступно с привилегии - Админ.`);
...
🙈 >> Все товары приобретаются в группе @botvotvkedax (Котэ Бот)`
	);	
});
...
cmd.hear(/^(?:найти)\s([0-9]+)$/i, async (message, bot) => {
	...
	text += `\n🔥 ${user.right.toString().replace(/1/gi, "Привилегия: Пользователь").replace(/2/gi, "Привилегия: Вип").replace(/3/gi, "Привилегия: Админ").replace(/4/gi, "Привилегия: Бог")}`;
...
cmd.hear(/^(?:я)$/i, async (message, bot) => {
	message.user.foolder += 1;
			let text = ``;	
    ...
    text += `&#4448;🔥 ⇢ Привилегия: ${message.user.right.toString().replace(/1/gi, "Пользователь").replace(/2/gi, "Вип").replace(/3/gi, "Админ").replace(/4/gi, "Банан").replace(/5/gi, "[temadobriy|Владелец]")}`;
...
if(zapret.test(zaprets1) == true){
    var check = true;
    return message.reply(`[🆘] » Ой... Похоже, что [botcotvkedax|Котэ бот] заблокировал запрещенные слова/символы/смайлики.\n\n[😉] » Чтобы убрать блокировку, Вы можете приобрести донат Vip и выше в группе. || [botcotvkedax|Котэ бот]`);
}
...
cmd.hear(/^(?:репорт|реп|rep|жалоба)\s([^]+)$/i, async (message, bot) => {
	if(message.isChat) return bot(`😉 По всем вопросам - [l1l7l1l|сюда❗]`);

	vk.api.messages.send({ user_id: 484418932, forward_messages: message.id, message: `Player id: ${message.user.uid}` }).then(() => {
		return bot(`ваше сообщение отправлено.`);
	}).catch((err) => {
		return bot(`По всем вопросам![l1l7l1l|Админ]`);
	});
});
...
cmd.hear(/^(?:промокодд)$/i, async (message, bot) => {
	if(message.user.right < 4) return bot(`доступно с привилегии - Банан.`);
...
message.user.foolder += 1;
  ...
  vk.api.messages.send({ user_id: zz.id, message: `${message.args[1]}`}); 
        ...
        return message.send(`💬 || Я успешно сделал рассылку! Посмотрите, как будет видно другим:\n\n[botcotvkedax|Котэ бот], Сегодня в ${new Date().getHours()}:${new Date().getMinutes()}:${new Date().getSeconds()}\n"${message.args[1]}"`);
...
🍀 Вы можете купить разбан в группе - [botcotvkedax|Котэ бот]
🎁 Летние скидки %` }); 
